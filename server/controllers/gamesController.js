...
const listTradeGames = asyncHandler(async (req, res) => {
    ...
    if (isPack) {
        ...
        // for want in trade
        for (let game of listedGames) {
            const userWantInTradeLists = await WantInTrade.find({ 'wantInTrade.bggId': game.bggId }).select('user').lean()
            ...
            for (let user of users) {
                notifArr.push({
                    ...
                    text: gameExists.isPack
                        ...
                        : `Price for one of your saved boardgames (${gameExists.games[0].title}) has changed to ${price} RON from ${oldPrice} RON`,
                    meta: {
...
const getSavedGames = asyncHandler(async (req, res) => {
    ...
    const savedGamesList = await User.findById({ _id: req.user._id })
        ...
        .populate({
            path: 'savedGames',
            select: '_id altId isActive isPack mode slug games.bggId games.year games.designers games.image games.thumbnail games.subtype games.title'
        })
        .lean()
    ...
    if (search) {
        ...
        const { savedGames } = await User.findOne({ _id: req.user._id })
            .select('savedGames')
            .populate({
                path: 'savedGames',
                options: {
...
const deleteOneGame = asyncHandler(async (req, res) => {
    ...
    for (let obj of game.games) {
        if (obj.userImage) {
            await storage.bucket(process.env.IMG_BUCKET).file(`f/${obj.userImage.name}`).delete({ ignoreNotFound: true })
            await storage.bucket(process.env.IMG_BUCKET).file(`t/${obj.userImage.name}`).delete({ ignoreNotFound: true })
        }
    }
        ...
        for (let user of users) {
            notifArr.push({
                ...
                text: game.isPack
                    ...
                    : `One of your saved boardgames (${game.games[0].title}) is no longer available for ${
                          game.mode === 'sell' ? 'sale' : 'trade'
                      }`,
                meta: {
