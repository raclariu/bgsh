...
export const useGetGamesIndexQuery = ({ sort, search, page, mode }) => {
    ...
    return useQuery(['index', mode, { sort, search, page }], () => api.apiGetGamesIndex({ sort, search, page, mode }), {
        staleTime: 1000 * 60 * 5,
        refetchOnMount: true,
        keepPreviousData: true,
        onError: (err) => {
...
export const useGetGameGalleryQuery = ({ altId, galleryInView, idx }) => {
    ...
    return useQuery(['singleGameScreen', 'gallery', { altId, idx }], () => api.apiFetchGallery(data.games[idx].bggId), {
...
export const useGetGameRecommendationsQuery = ({ altId, recsInView, idx }) => {
    ...
    return useQuery(['singleGameScreen', 'recs', { altId, idx }], () => api.apiFetchRecommendations(data.games[idx].bggId), {
