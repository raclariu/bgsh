...
export const apiGetGamesIndex = async ({ search, page, sort, mode }) => {
    const config = {
        params: {
            ...
            page: +page ? +page : 1,
            sort: sort ? sort : 'reactivated',
            mode
        }
    }
    const { data } = await axsPUBLIC.get('/api/games', config)
...
export const apiFetchSingleGame = async (altId) => {
    const { data } = await axsPUBLIC.get(`/api/games/${altId}`)
    return data
}
...
export const apiFetchGallery = async (bggId) => {
    ...
    const { data } = await axsPUBLIC.get('/api/misc/bgg/gallery', config)
    return data
}
...
export const apiFetchRecommendations = async (bggId) => {
    ...
    const { data } = await axsPUBLIC.get('/api/misc/bgg/recommendations', config)
    return data
}
...
export const apiFetchVideos = async (bggId) => {
    ...
    const { data } = await axsPUBLIC.get('/api/misc/bgg/videos', config)
    return data
}
...
export const apiGetProfileData = async (username) => {
    const { data } = await axsPUBLIC.get(`/api/users/${username}`)
    return data
}

export const apiGetProfileListingsData = async (username) => {
    const { data } = await axsPUBLIC.get(`/api/users/${username}/listings`)
    return data
}
