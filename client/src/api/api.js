...
export const apiSendConvMessage = async (id, text) => {
	const config = {
		headers : {
			'Content-Type' : 'application/json',
			Authorization  : getBearer()
	...
	await axios.post(`/api/messages/conversations/${id}`, { text }, config)
}

export const apiGetSingleConv = async (page, id) => {
	const config = {
		headers : {
			'Content-Type' : 'application/json',
			Authorization  : getBearer()
	...
	const { data } = await axios.get(`/api/messages/conversations/${id}`, config)
	return data
}
