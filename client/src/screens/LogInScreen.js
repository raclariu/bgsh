                ...
                <form onSubmit={submitHandler} autoComplete='off'>
                    <DialogTitle>
                        ...
                        <Box textAlign='center' mt={1} color='text.secondary' fontSize='subtitle2.fontSize'>
                            An email will be sent to your email address containing instructions on how to reset your password
                        </Box>
                    </DialogTitle>

                    <DialogContent dividers>
                        <Input
                            error={isError && error.response.data.message ? error.response.data.message.emailError : false}
                            helperText={
...
const LogIn = ({ isActivationPage }) => {
    ...
    const {
        ...
        mutate,
        error
    } = useMutation((credentials) => apiUserLogin(credentials), {
        retry: 0,
        onError: (err) => {
