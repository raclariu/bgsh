...
// @ Others
import { useDeleteFromListMutation, useGetListQuery, useGetBggGamesDetailsQuery, useListGamesMutation } from '../hooks/hooks'
...
const SellGamesScreen = () => {
    ...
    return (
        <form onSubmit={handleSubmit} autoComplete='off'>
            ...
            {userList.isSuccess && userList.data.list.length === 0 && (
                <Box my={2}>
                    <CustomAlert severity='warning'>Your list is empty</CustomAlert>
                </Box>
            )}

            {userList.isSuccess && userList.data.list.length > 0 && isSuccessDetails && (
                <Fragment>
                    ...
                    >
                        <FormControl required error={shipError} fullWidth>
                            ...
                            <FormGroup>
                                ...
                                {shipError && <FormHelperText error>Choose at least one shipping method</FormHelperText>}
                            </FormGroup>
                        </FormControl>
                        ...
                        {isPack && (
                            <Fragment>
                                <Input
                                    ...
                                    value={totalPrice}
                                    error={listMutation.isError && listMutation.error.response.data.message.totalPrice}
                                    helperText={listMutation.isError && listMutation.error.response.data.message.totalPrice}
                                    name='total-price'
                                    label='Pack price'
                                    type='number'
                                    InputProps={{
