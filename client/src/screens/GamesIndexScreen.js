...
const GamesIndexScreen = ({ mode }) => {
    ...
    const { search, sort = 'reactivated', page = 1 } = queryString.parse(location.search)
    ...
    return (
        <Fragment>
            <Helmet
                title={
                    mode === 'sell' ? 'Boardgames for sale' : mode === 'trade' ? 'Boardgames for trade' : 'Wanted boardgames'
                }
            />
            ...
            <Box display='flex' width='100%' mb={3} justifyContent='center' alignItems='center'>
                <Grid container justifyContent='center' spacing={2}>
                    <Grid item md={4} sm={6} xs={12}>
                        <SearchBox
                            placeholder={
                                mode === 'sell' ? 'Search sales' : mode === 'trade' ? 'Search trades' : 'Search wanted games'
                            }
            ...
            <Box display='flex' width='100%' justifyContent='space-between' alignItems='flex-end' mb={2} flexWrap='wrap'>
                <Box>
                    {isSuccess && search && (
            ...
            {isSuccess && data.gamesData.length === 0 && (
